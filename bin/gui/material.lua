...
local cur_data, material, property

local clear_fields, fields, shader
...
local function clear_prop_fields()
	for _, field in pairs(fields) do
		field.value = ""
	end
end
...
local function read(data)
	clear()
	cur_data = data.read.mod.materials
	mat_list.autoredraw = "NO"
	mat_list[1] = nil
	for i, mat in ipairs(cur_data) do
		mat_list[i] = mat.name
	end
...
return {
	...
	display = iup.hbox{
		...
		iup.frame{title = "Properties", iup.vbox{prop_list; nmargin = "5x5"}},
		iup.vbox{iup.frame{title = "Property Values", iup.vbox{grid, nmargin = "10x10"}},
		iup.frame{title = "Material Shader", iup.vbox{shader, nmargin = "10x10"}}, gap = 10, nmargin = "10x0"}; nmargin = "10x10", --[[gap = 10]]},
	read = read,
	clear = clear,
}
